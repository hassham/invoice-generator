...
public class BusinessEndpointsTests
{
    ...
    // IG-46: GenerateNextInvoiceNumberAsync now issues Postgres-specific raw SQL (UPDATE ...
    // RETURNING) for atomicity, which the InMemory provider every other test in this file uses
    // can't execute at all - this test alone needs a real Npgsql connection, reusing the same
    // invoiceapp-postgres docker container the app uses in local development. Skips (not fails)
    // when Postgres isn't reachable, same reasoning as PostgresAvailabilityFixture in
    // InvoiceApp.Infrastructure.Tests: this repo's CI backend job has no Postgres service
    // container, so a hard failure here would break every push.
    ...
    public async Task Generates_a_formatted_next_invoice_number_and_increments_it()
    {
        ...
        var email = $"numbering-generate-{Guid.NewGuid():N}@example.com";
